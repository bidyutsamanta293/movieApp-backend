...
(async () => {
    ...
    // Create seed data.
    const doctorRepo = conn.getRepository(ScreenSeat);
    let screenseat = new ScreenSeat();
    screenseat.seat = 'A2';
    screenseat.screen = 1;
    screenseat = await doctorRepo.save(screenseat); // re-assign to know assigned id
    console.log(`\nDoctor saved. id = ${screenseat.id}`);
